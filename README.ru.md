...
*Интеллектуальная платформа автоматизации, превращающая требования в отчеты о тестировании*
...
graph LR
    ...
    Runner <-->|Визуальное Взаимодействие| Phone[📱 Мобильное Устройство]
...
1.  **Только Android**: В настоящее время поддерживаются только устройства Android (через ADB). iOS не поддерживается.
2.  **Скорость Выполнения**: Из-за интенсивного использования VLM, вывод занимает около 2-5 секунд на шаг, что медленнее, чем нативные скрипты Appium/Espresso.
3.  **Область Проверки**: В основном поддерживается визуальная проверка UI (текст/иконки). Проверка состояния базы данных или ответов API пока не поддерживается.
...
- **Android Устройство** (Реальное устройство или Эмулятор с включенным ADB)
...
# Установка зависимостей (с поддержкой Zhipu AI)
