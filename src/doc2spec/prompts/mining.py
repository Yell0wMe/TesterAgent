...
1. **全面覆盖**：针对每个功能点，必须从以下三个维度拆解需求：
   ...
   - 🚧 **边界场景 (Boundary)**: 极值输入（如超长文本、0金额）、临界状态（如列表为空、最后一条数据）。
2. **上下文感知**：必须分析文档标题和上下文，明确提取 `target_app`（目标应用）、`target_url`（Web URL）和 `target_page`（所在页面）。
3. 只提取文档中明确描述或强暗示的需求，禁止无依据的脑补。
4. 每个需求项必须独立、原子化，**不要把正向和逆向场景合并在一个条目中**，请拆分为多条。
5. 每个需求必须有明确的、可在 UI 上观察到的成功态（Text/Element）。
6. 输出必须是严格的 JSON 数组格式。

**输出字段说明：**
- req_id: 需求编号
- req_title: 需求标题
- user_goal: 用户目标描述
- target_app: 目标应用名称（如 '京东APP', '微信', 必填）
- target_url: 目标 Web URL (如 'https://m.jd.com', 若是 Web 项目必填)
- target_page: 目标页面/入口（如 '首页>搜索', '个人中心'）
- env_specs: 环境/设备要求
- success_ui: 成功态 UI 表现
- explicit_steps: 明确步骤
- preconditions: 前置条件
